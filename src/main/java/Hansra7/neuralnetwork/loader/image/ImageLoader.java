...
public class ImageLoader implements Loader {
	...
	private ImageMetaData readMetaData() {
		...
		try {
			...
			if (magicLabelNumber != 2049) {
				throw new LoaderException("Label file " + labelFileName + " has wrong format.", null);
			}
		...
		try {
			...
			if (magicImageNumber != 2051) {
				throw new LoaderException("Image file " + imageFileName + " has wrong format.", null);
			}

			if (dsImages.readInt() != numberItems) {
				throw new LoaderException("Image file " + imageFileName + " has different number of items to " + labelFileName, null);
			}
	...
	public BatchData readBatch() {
		...
		try {
			...
			if(inputItemsRead != expectedItemsRead) {
				throw new LoaderException("Mismatch between images read and labels read.", null);
			}
	...
	private int readExpectedBatch(ImageBatchData batchData) {
		try {
			var totalItemsRead = metaData.getTotalItemRead();
			var numberItems = metaData.getNumberItems();

			var numberToRead = Math.min(numberItems - totalItemsRead, batchSize);

			var labelData = new byte[(int) numberToRead];
			var expectedSize = metaData.getExpectedSizes();

			var numberRead = dsLabels.read(labelData, 0, numberToRead);

			if (numberRead != numberToRead) {
				throw new LoaderException("Couldn't read sufficient bytes from image data", null);
			}
